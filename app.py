...
def ensure_column(connection, table_name, column_name, ddl):
    cols = [r["name"] for r in connection.execute(f"PRAGMA table_info({table_name})").fetchall()]
    if column_name not in cols:
        connection.execute(f"ALTER TABLE {table_name} ADD COLUMN {ddl}")


def migrate_schema(connection):
    ensure_column(connection, "users", "cheat_flags", "cheat_flags INTEGER DEFAULT 0")
    ensure_column(connection, "users", "created_at", "created_at TEXT")
    ensure_column(connection, "rounds", "started_at", "started_at INTEGER DEFAULT 0")
    ensure_column(connection, "questions", "qtype", "qtype TEXT DEFAULT 'DSA'")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_api_post(self, parsed):
        ...
        if parsed.path == "/api/anti-cheat":
            ...
            should_disqualify = tab_switches > 3 or total_flags >= 10
